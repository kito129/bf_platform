...
let RuleEditComponent = class RuleEditComponent {
    ...
    ngOnInit() {
        const rule = new _model_study_new_rule_rules__WEBPACK_IMPORTED_MODULE_4__["Rule"](0);
        this.myForm = this.fb.group({
            name: ['', [
                    _angular_forms__WEBPACK_IMPORTED_MODULE_3__["Validators"].required,
                ]],
            rule: this.fb.array([this.generateNewRule(rule)], [
                _angular_forms__WEBPACK_IMPORTED_MODULE_3__["Validators"].required,
            ])
        });
    }
    ...
    addRule() {
        const temp = new _model_study_new_rule_rules__WEBPACK_IMPORTED_MODULE_4__["Rule"](this.rules.length + 1);
    ...
    generateSubTrades(data) {
        ...
        for (const trades of data.map(x => x.trades)) {
            ...
            for (const trade of trades) {
                ...
                tempRR.push(+(trade.trade.result.rr).toFixed(2));
                // tradeComparator
                // @ts-ignore
        ...
        * SUBSCRIBE TO STATE
        ...
        Object(rxjs__WEBPACK_IMPORTED_MODULE_3__["combineLatest"])([this.selectedStudyMarketTrade$, this.selectedStudyMarket$,])
            ...
            if (data[0] && data[1]) {
                ...
                for (const backTrade of trade.trade.trades.back) {
                ...
                for (const layTrade of trade.trade.trades.lay) {
                    const selectionN = layTrade.selectionN;
                    this.selectedTrades.push({
                        isBackTrade: false,
                        odds: layTrade.odds,
                        stake: layTrade.bank,
