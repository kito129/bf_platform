...
export class TennisPointPipe implements PipeTransform {
  ...
  transform(point: TennisPoint){

    const set = checkSet(point)

    switch (set){
      case (0):{
        return '-' + appendGame(point) + appendServer(point)
      }
      case (1):{
        return point.set1.runnerA.toString()+  point.set1.runnerB.toString() + appendGame(point) + appendServer(point)
      }
      case (2):{
        return point.set1.runnerA.toString() + point.set1.runnerB.toString() + ' - ' + point.set2.runnerA.toString() + point.set2.runnerB.toString()
          + appendGame(point) + appendServer(point)
      }
      case (3):{
        return point.set1.runnerA.toString() + point.set1.runnerB.toString() + ' - ' + point.set2.runnerA.toString() + point.set2.runnerB.toString()
...
function appendGame(point: TennisPoint){
  if(!((point.currentGame.runnerA=== '0' || point.currentGame.runnerA=== null) && (point.currentGame.runnerB=== '0' || point.currentGame.runnerB=== null))){
...
function appendServer(point: TennisPoint){
  if(point.currentGame.server==='A' || point.currentGame.server==='B'  ){
