...
export class TradeUpdatesModalComponent implements OnInit {
  ...
  * SELECTIONS
  ...
  public addSelection(){
    ...
    this.tradeOutput.trade.exposition.push({
      back: null,
      lay: null,
      selectionN: this.tradeOutput.trade.selections.length-1,
    })
  ...
  public removeSelection(selectionIndex: number) {
    if (selectionIndex > -1) {
      ...
      this.tradeOutput.trade.exposition.splice(selectionIndex, 1);
      ...
      let h =0
      for (const trade of this.tradeOutput.trade.trades.lay){
        if(trade.selectionN === selectionIndex){
          this.tradeOutput.trade.trades.lay.splice(k, 1);
          h--
        }
        h++
      }
